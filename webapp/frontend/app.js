...
class ResumeEditor {
    constructor() {
        ...
        this.files = new Map();
        this.fileMtimes = new Map(); // Track file modification times
        this.autoCompile = false;
        this.autoSave = localStorage.getItem('autoSave') === 'true'; // Default OFF
        this.yamlMode = localStorage.getItem('yamlMode') === 'true';
        this.vimMode = localStorage.getItem('vimMode') === 'true';
        this.vimAdapter = null; // monaco-vim adapter instance
        this.vimStatusNode = null; // Status bar for Vim mode
        this.compileTimeout = null;
        this.fileWatchInterval = null; // Interval for checking backend file changes
        this.pdfDoc = null;
        this.scale = 1.0;
        this.pageNum = 1;
        this.rotation = 0;
        ...
        this.pendingSave = false; // Track if there are unsaved changes
    ...
    async initializeApp() {
        try {
            ...
            await this.loadPdf();
            this.startFileWatcher(); // Start watching for backend file changes
        ...
        try {
            ...
            for (const filename of data.resume_files) {
                ...
                if (fileResponse.ok) {
                    ...
                    this.fileMtimes.set(filename, fileData.mtime || 0);
                }
            }
    ...
    saveCurrentFile() {
        ...
        fetch(`/api/files/${this.currentFile}`, {
            ...
            if (response.ok) {
                // Update mtime to current time to prevent reload
                this.fileMtimes.set(this.currentFile, Date.now() / 1000);
    ...
    toggleAutoSave() {
        ...
        btn.textContent = `💾 Save: ${this.autoSave ? 'Auto' : 'Manual'}`;
        btn.className = `btn ${this.autoSave ? 'btn-primary' : 'btn-secondary'} btn-sm`;
        btn.title = this.autoSave ? 'Auto-save enabled' : 'Manual save (Ctrl+S)';

        // If turning on auto-save and there are pending changes, save now
        if (this.autoSave && this.pendingSave) {
            this.saveCurrentFile();
        }
    }

    // File watcher for backend changes
    startFileWatcher() {
        // Poll every 2 seconds for file changes
        this.fileWatchInterval = setInterval(() => this.checkForFileChanges(), 2000);
    }
    ...
    async checkForFileChanges() {
        try {
            const response = await fetch('/api/files/timestamps');
            ...
            for (const [filename, newMtime] of Object.entries(timestamps)) {
                const oldMtime = this.fileMtimes.get(filename) || 0;

                // If file was modified externally (mtime increased and we didn't just save it)
                if (newMtime > oldMtime + 0.5) { // 0.5 second buffer
                    console.log(`File changed externally: ${filename}`);
                    filesChanged = true;
                    ...
                    if (fileResponse.ok) {
                        const fileData = await fileResponse.json();
                        this.files.set(filename, fileData.content);
                        this.fileMtimes.set(filename, fileData.mtime);
                        ...
                        if (filename === this.currentFile) {
                            ...
                            this.pendingSave = false;
                            this.updateSaveIndicator();
                            this.showNotification(`${filename} updated from disk`);
                        }
                    }
                }
            }
            ...
            // Check for new files
            for (const filename of Object.keys(timestamps)) {
            ...
            console.debug('File watch poll failed:', error);
        }
    }
    ...
    async compile() {
        // Route to YAML compilation if YAML mode is enabled
        if (this.yamlMode) {
            return this.compileFromYaml();
        }

        try {
            this.updateStatus('compiling', 'Compiling...');
            this.showLogPanel();
            this.updateLog('Starting compilation...\n');
            ...
            if (data.success) {
                ...
                // Auto-save snapshot after successful compilation
    ...
    async compileFromYaml() {
        try {
            this.updateStatus('compiling', 'Compiling from YAML...');
            this.showLogPanel();
            this.updateLog('Starting compilation from YAML...\n');

            const response = await fetch('/api/compile-from-yaml', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
            ...
            if (data.success) {
                this.updateStatus('success', 'Compiled successfully from YAML');
                this.updateLog('\n✅ Compilation from YAML successful!\n');
                // Update viewing file to main CV when PDF is compiled
                this.viewingFile = 'cv-llt.tex';
                this.buildFileTree(); // Refresh tree to show viewing indicator
                await this.loadPdf();
                ...
                // Refresh file contents in editor if any .tex file is open
                if (this.currentFile && this.currentFile.endsWith('.tex')) {
                    await this.loadFile(this.currentFile);
                }
            } else {
                this.updateStatus('error', data.error || 'Compilation from YAML failed');
                this.updateLog('\n❌ Compilation from YAML failed: ' + (data.error || 'Unknown error') + '\n');
            }

        } catch (error) {
            console.error('Compilation from YAML error:', error);
            this.updateStatus('error', 'Network error');
            this.updateLog('\n❌ Network error: ' + error.message + '\n');
        }
    }
    ...
    updateLog(message) {
        ...
        if (logContent) {
            // If this is a fresh compile (starts with "Starting"), clear previous log
    ...
    showLogPanel() {
        ...
        mainContainer.classList.remove('log-collapsed');
    ...
    toggleLogPanel() {
        ...
        if (logPanel.classList.contains('collapsed')) {
            ...
            mainContainer.classList.add('log-collapsed');
            logToggle.textContent = '▲';
        }
    }
    ...
    async loadPdf() {
        try {
            ...
            const response = await fetch(`/api/pdf?t=${cacheBuster}`);
    ...
    toggleYamlMode() {
        ...
        localStorage.setItem('yamlMode', this.yamlMode.toString());
        const btn = document.getElementById('yaml-mode-btn');
        btn.textContent = `📝 YAML: ${this.yamlMode ? 'ON' : 'OFF'}`;
        btn.className = `btn ${this.yamlMode ? 'btn-primary' : 'btn-secondary'} btn-sm`;
        btn.title = this.yamlMode ? 'YAML mode enabled - Compile generates from YAML' : 'YAML mode disabled - Compile uses .tex files';
    }
    ...
    // Vim Mode functionality
    toggleVimMode() {
        this.vimMode = !this.vimMode;
        localStorage.setItem('vimMode', this.vimMode.toString());
        const btn = document.getElementById('vim-mode-btn');
        btn.textContent = `⌨️ Vim: ${this.vimMode ? 'ON' : 'OFF'}`;
        btn.className = `btn ${this.vimMode ? 'btn-primary' : 'btn-secondary'} btn-sm`;
        btn.title = this.vimMode ? 'Vim keybindings enabled' : 'Vim keybindings disabled';
    ...
    enableVimMode() {
        ...
        if (typeof window.MonacoVim === 'undefined') {
            console.warn('monaco-vim not loaded, using basic Vim simulation');
            this.editor.updateOptions({ cursorStyle: 'block' });
            this.showVimModeIndicator('-- VIM MODE (BASIC) --');
            return;
        }

        try {
            // Create status node for Vim mode feedback
            this.createVimStatusNode();

            // Initialize monaco-vim
            this.vimAdapter = window.MonacoVim.initVimMode(
                this.editor,
                this.vimStatusNode
            );

            console.log('Full Vim keybindings enabled');
        } catch (error) {
    ...
    disableVimMode() {
        ...
        // Remove status node
        if (this.vimStatusNode && this.vimStatusNode.parentNode) {
            this.vimStatusNode.parentNode.removeChild(this.vimStatusNode);
            this.vimStatusNode = null;
        }
        ...
        console.log('Vim keybindings disabled');
    }
    ...
    createVimStatusNode() {
        ...
        this.vimStatusNode.id = 'vim-status-bar';
        ...
        if (editorContainer) {
            ...
            editorContainer.appendChild(this.vimStatusNode);
        }
    }

    showVimModeIndicator(text) {
        // Fallback indicator for basic mode
    ...
    downloadResume(format) {
        if (format === 'pdf') {
            ...
            // Generate formatted Word document from YAML
            this.generateWordDoc(false);
        } else if (format === 'docx-ats') {
            // Generate ATS-friendly barebones Word document
            this.generateWordDoc(true);
        }
    }

    async generateWordDoc(atsMode = false) {
        try {
            this.updateStatus('compiling', atsMode ? 'Generating ATS Word Doc...' : 'Generating Word Doc...');
            this.showLogPanel();
            this.updateLog(atsMode ? 'Generating ATS Word document...\n' : 'Generating formatted Word document...\n');
            ...
            a.href = url;
            a.download = atsMode ? 'resume-ats.docx' : 'resume.docx';
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);

            this.updateStatus('success', 'Word document generated');
            this.updateLog('\n✅ Word document generated successfully!\n');

        } catch (error) {
            console.error('Word generation error:', error);
            this.updateStatus('error', 'Failed to generate Word document');
            this.updateLog('\n❌ Error: ' + error.message + '\n');
        }
    }
    ...
    async saveVersion() {
        try {
            ...
            this.updateStatus('compiling', 'Saving version...');
            ...
            const data = await response.json();
            this.updateStatus('success', `Version saved: ${data.snapshot_id}`);
            ...
            this.updateStatus('error', 'Failed to save version');
        }
    }

    async autoSaveSnapshot() {
        // Silently save snapshot in background after successful compilation
        try {
            await fetch('/api/snapshots/save', {
                method: 'POST'
            });
            console.log('Auto-snapshot saved');
        } catch (error) {
    ...
    async loadVersionsList() {
        try {
            ...
            if (data.snapshots.length === 0) {
                versionsList.innerHTML = '<div class="text-sm text-gray-500 p-2">No saved versions yet</div>';
                return;
            }

            // Build versions list
            versionsList.innerHTML = data.snapshots.map(snapshot => `
                <button class="dropdown-item" data-snapshot-id="${snapshot.id}">
                    <div class="font-medium">${snapshot.date}</div>
                    <div class="text-xs text-gray-500">${snapshot.files.length} files</div>
                </button>
            `).join('');

            // Add click handlers
            versionsList.querySelectorAll('.dropdown-item').forEach(item => {
                item.onclick = (e) => {
                    e.stopPropagation();
                    const snapshotId = item.dataset.snapshotId;
                    this.restoreVersion(snapshotId);
                    document.getElementById('versions-menu').classList.remove('show');
                };
            });
            ...
            const versionsList = document.getElementById('versions-list');
            versionsList.innerHTML = '<div class="text-sm text-red-500 p-2">Error loading versions</div>';
        }
    }

    async restoreVersion(snapshotId) {
        if (!confirm(`Restore version from ${snapshotId}? Current changes will be overwritten.`)) {
            return;
        }

        try {
            this.updateStatus('compiling', 'Restoring version...');
            ...
            this.updateStatus('success', `Version restored: ${data.files_restored.length} files`);
            ...
            this.updateStatus('error', 'Failed to restore version');
        }
    }
    ...
    setupEventListeners() {
        ...
        // YAML mode toggle
        document.getElementById('yaml-mode-btn').onclick = () => this.toggleYamlMode();

        // Vim mode toggle
        document.getElementById('vim-mode-btn').onclick = () => this.toggleVimMode();

        // Auto-compile toggle
        document.getElementById('auto-compile-btn').onclick = () => this.toggleAutoCompile();

        // Auto-save toggle
        document.getElementById('auto-save-btn').onclick = () => this.toggleAutoSave();

        // Initialize auto-save button state
        const autoSaveBtn = document.getElementById('auto-save-btn');
        autoSaveBtn.textContent = `💾 Save: ${this.autoSave ? 'Auto' : 'Manual'}`;
        autoSaveBtn.className = `btn ${this.autoSave ? 'btn-primary' : 'btn-secondary'} btn-sm`;
        autoSaveBtn.title = this.autoSave ? 'Auto-save enabled' : 'Manual save (Ctrl+S)';

        // Initialize YAML mode button state (update UI without toggling state)
        const yamlBtn = document.getElementById('yaml-mode-btn');
        yamlBtn.textContent = `📝 YAML: ${this.yamlMode ? 'ON' : 'OFF'}`;
        yamlBtn.className = `btn ${this.yamlMode ? 'btn-primary' : 'btn-secondary'} btn-sm`;
        yamlBtn.title = this.yamlMode ? 'YAML mode enabled - Compile generates from YAML' : 'YAML mode disabled - Compile uses .tex files';

        // Initialize Vim mode button state (update UI without toggling state)
        const vimBtn = document.getElementById('vim-mode-btn');
        vimBtn.textContent = `⌨️ Vim: ${this.vimMode ? 'ON' : 'OFF'}`;
        vimBtn.className = `btn ${this.vimMode ? 'btn-primary' : 'btn-secondary'} btn-sm`;
        vimBtn.title = this.vimMode ? 'Vim keybindings enabled' : 'Vim keybindings disabled';
        ...
        versionsBtn.onclick = (e) => {
            e.stopPropagation();
            versionsMenu.classList.toggle('show');
            if (versionsMenu.classList.contains('show')) {
        ...
        document.addEventListener('click', (e) => {
            if (!versionsBtn.contains(e.target) && !versionsMenu.contains(e.target)) {
                versionsMenu.classList.remove('show');
            }
        });
        ...
        // Log panel toggle
        document.getElementById('log-toggle').onclick = () => this.toggleLogPanel();
            ...
            if (e.ctrlKey || e.metaKey) {
                switch (e.key) {
                    ...
                    case 'z':
                        ...
                        // Ctrl+Z for Undo is handled by Monaco natively
                        break;
                    case 'y':
